...
| ProcVal(params, body, env) -> 
  ...
  if param_len != vals_len
    then raise @@ InterpreterError ("Arity mismatch. Expected " ^ Int.to_string param_len ^ ", Given: " ^ Int.to_string vals_len)
    else let val_list = List.map2 (fun s v -> s,v) params vals
    in value_of body @@ ext_env env val_list
| _ -> raise @@ InterpreterError ("Unable to find <func " ^ name ^ "> in enviroment")
...
and value_of_binary op exp1 exp2 env =
  ...
  | OR, v1, v2 -> 
    ...
    let t2 = is_truthy v2 in BoolVal(t1 || t2)
  | AND, v1, v2 -> 
    let t1 = is_truthy v1 in 
    let t2 = is_truthy v2 in BoolVal(t1 && t2)
